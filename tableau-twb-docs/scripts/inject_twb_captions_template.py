...
def resolve_shelf(raw, fm):
    ...
    for ds, cp in re.findall(r'\[([^\]]+)\]\.\[([^\]]+)\]', raw):
        full = f"[{ds}].[{cp}]"
        name = resolve(full, fm)
        am = re.match(r'([a-z]+):(.+):([a-z]+)', cp)
        agg = agg_label(am.group(1)) if am else ""
...
def _resolve_table_calc(tc, parent_map, fm):
    ...
    return kind, field_name, formula, ordering, tc_field, order_children
...
def gen_sections(ws, fm, formula_map, action_idx, parent_map):
    ...
    for tc in ws.findall(".//table-calc"):
        kind, field_name, formula, ordering, tc_field, order_children = \
            _resolve_table_calc(tc, parent_map, fm)
        if kind == "shelf":
            shelf_groups.setdefault(field_name, []).append((ordering, tc_field, order_children))
    ...
    if shelf_groups:
        ...
        for fname, configs in shelf_groups.items():
            ...
            default_dir = None
            sort_field = None
            for ordering, tc_field, order_children in configs:
                if ordering == "Field":
            ...
            if specific_dims:
                S.append(("detail", f"        [{fname}]: Compute Using Specific Dimensions"))
                S.append(("detail", f"            Checked: {', '.join(specific_dims)}"))
                if sort_field:
                    S.append(("detail", f"            Sort order: Specific Dimensions (by [{sort_field}])"))
            elif default_dir:
                S.append(("detail", f"        [{fname}]: Compute Using {default_dir}"))
        S.append(("brk", ""))
